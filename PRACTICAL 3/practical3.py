...
members_per_group = int(input())            # take numbers of members per groups
room_numbers = list(map(int, input().strip().split()))  # taking input of room numbers of members
room_numbers = sorted(room_numbers)         # convert list into the sorted list
length = len(room_numbers)                  # assign length of list
for i in range(length):
    if i != length - 1:
        if (room_numbers[i] != room_numbers[i + 1]) and (room_numbers[i] != room_numbers[i - 1]):
            print(room_numbers[i])          # print the result
            break
    else:
        print(room_numbers[i])              # print the result
